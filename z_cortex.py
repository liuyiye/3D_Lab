...
def w():
    ...
    maskVolumeNode = slicer.util.getNode("synthseg")
    referenceVolumeNode = slicer.util.getNode("lesion_L01_111")
    outputVolumeNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLScalarVolumeNode", "synthseg_111")
    ...
    if mask.max() != 154:
       print('mask is not 154, please check')
       return()
    if mask.max() > 64:
    ...
    synthseg_111_node=slicer.util.getNode('synthseg_111')
    synthseg_111=slicer.util.arrayFromVolume(synthseg_111_node)
    
    flair[(synthseg_111==42) | (synthseg_111==3)]=0 #删除可能出现在皮层的错误病灶
